[english version follows at the bottom]

                              Gratulation!
                           ******************
 
   * Dein Telefonanschluss wurde soeben auch fuer abgehende Amtsgespraeche
     freigeschaltet.
   * Das Amt kannst Du erreichen, indem Du zuerst die 0 waehlst. Dann
     hoerst Du einen Dauerton. Jetzt wie in einer Telefonzelle waehlen.
     (Bsp.: zu mir nach hause waehle ich 0 072718xxx)   
   * Notruf ist bei ALLEN Telefonen (ausser am Klingelboard) IMMER ueber
     <*112> zu erreichen.
   * An diese (deine) Email-Adresse werden auch die
     Einzelverbindungsnachweise monatlich geschickt.

Sollte dennoch etwas nicht klappen, einfach beim HEKphone-Team melden.

mailto: telefon\@hek.uni-karlsruhe.de
oder  : Tel: 1234 (Sammelruf) oder persoenlich vorbeischauen.

Ausfuehrliche Anleitung siehe http://hekphone.uni-karlsruhe.de/

Bitte ueberpruefe, ob sich auch kein Tippfehler in deiner Kontoverbindung eingeschlichen hat.
Dies wuerde zu einer Ruecklastschrift fuehren, die mindestens 10 Euro Gebuehren verursacht.

Kontonummer   : <?php echo $bank_number ?>
Bankleitzahl  : <?php echo $account_number ?>
Rechnungsemail: <?php echo $email ?>

------------------------------------------------------------------------------------
   
                            Congratulations
                        ************************
     
   * Your HEK-phone has been enabled to dial outgoing calls.
   * To get outside line access you first have to dial 0 then you'll hear a 
     continuous tone. Now go on as if you are in a telephonebox. (example.:
     I want to call my family. So I call this number  0 072718xxx).
   * Emergency call is <*112> with ALL phones (except the one at the
     ringer-board) at EVERY TIME.
   * This (your) email-address is where you get a list of every call you
     had made monthly.

In case of problems please contact the HEK-phone team.
mailto: telefon\@hek.uni-karlsruhe.de
or    : Tel: 1234 (multiple-subscribers!) or come by yourself


If you need manuals for quite everything please visit our homepage
http://hekphone.uni-karlsruhe.de

Please check that there is no typo in your bank account details. This would
cause your bank not to fullfill our withdrawls and would lead to costs at a minimum 
of 10 Euro.

Your bank account number (\"Kontonummer\"): <?php echo $bank_number ?>
Bank number (\"Bankleitzahl, BLZ\")       : <?php echo $account_number ?>
E-mail account for your invoices        : <?php echo $email ?>